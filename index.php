...
<?php else: ?>
    ...
    <?php else: ?>
            ...
            <?php if ($match): ?>
                <?php
                    ...
                    foreach ($match['players'] as $p) {
                        if (isset($p['account_id']) && (string)$p['account_id'] === $user['steam_id']) { $player = $p; break; }
                    }
                ?>
        ...
        <?php else: ?>
            ...
            <?php else: ?>
            <div class="mb-3">
                <input type="text" id="heroFilter" class="form-control" placeholder="Filter by hero">
                <div class="form-check mt-2">
                    <input class="form-check-input" type="checkbox" id="notesOnly">
                    <label class="form-check-label" for="notesOnly">Only matches with notes</label>
                </div>
            </div>
            ...
            ?>
                    <tr data-hero="<?=htmlspecialchars($hero['localized_name'] ?? '')?>" data-notes="<?=$noteCount?>">
                        <td><?=$m['match_id']?></td>
                        <td>
                            <?php if ($img): ?><img src="<?=$img?>" alt="" style="height:32px" class="me-2"><?php endif; ?>
                            <?=htmlspecialchars($hero['localized_name'] ?? 'Unknown')?>
                        </td>
                        <td><?=$m['kills']?>/<?=$m['deaths']?>/<?=$m['assists']?></td>
                        <td><?=$noteCount?></td>
                        <td><a class="btn btn-sm btn-primary" href="index.php?page=match&match_id=<?=$m['match_id']?>">View</a></td>
                    </tr>
            <?php endforeach; ?>
                </tbody>
            </table>
            ...
            function applyFilter(){
                const term=heroFilter.value.toLowerCase();
                const notes=notesOnly.checked;
                document.querySelectorAll('#matchesTable tbody tr').forEach(tr=>{
                    const hero=tr.dataset.hero.toLowerCase();
                    const noteCount=parseInt(tr.dataset.notes,10);
                    const heroMatch=hero.includes(term);
                    const notesMatch=!notes||noteCount>0;
                    tr.style.display=(heroMatch&&notesMatch)?'':'none';
                });
            }
            heroFilter.addEventListener('input',applyFilter);
            notesOnly.addEventListener('change',applyFilter);
            </script>
            <?php endif; ?>
        <?php endif; ?>
    <?php endif; ?>
